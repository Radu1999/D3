...
def recursively_read(rootdir, must_contain, exts=["png", "jpg", "jpeg", "PNG", "JPEG", "webp", "WEBP"]):
    out = []
    for r, d, f in os.walk(rootdir):
        for file in f:
            if (file.rsplit('.', 1)[-1] in exts)  and  (must_contain in os.path.join(r, file)):
...
class RealFakeDataset(Dataset):
    def __init__(self, opt, real_folders, fake_folders):
        ...
        for path in real_folders:
            real_list = real_list + get_list(path)
        for path in fake_folders:
            fake_list = fake_list + get_list(path)
        ...
        elif '2b' not in opt.arch:
            # print (f"using Official CLIP's normalization")
            self.transform = transforms.Compose([
                rz_func,
